									...
									<?php foreach ($ShowAdmin->result_array() as $row):
										...
										<tr class="table-row-hover">
											...
											<td class="text-center align-middle PrintExclude">
												...
												<button type="button" class="btn btn-primary btn-sm w-100 admin-moreinfo-btn mb-1" data-toggle="modal" data-target="#AdminMoreInfoModal" data-adminid="<?php echo $row['AdminID']; ?>" data-adminimage="<?php echo $row['Image']; ?>" data-adminposition="<?php echo $row['Position']; ?>" data-adminpermissions="<?php echo $row['Permissions']; ?>" data-adminlastname="<?php echo $row['LastName']; ?>" data-adminfirstname="<?php echo $row['FirstName']; ?>" data-adminmiddlename="<?php echo $row['MiddleName']; ?>" data-adminnotes="<?php echo $row['Notes']; ?>" <?php if($isActivityNotEmpty): ?>data-activityelapsed="<?php echo $activityElapsed->diffForHumans(); ?>"<?php endif; ?>><i class="fas fa-address-card"></i> More Info</button>
												<?php if($this->Model_Security->CheckPermissions('AdminsEditing')): ?>
