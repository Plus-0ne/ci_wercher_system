		...
		<div id="content" class="ncontent">
			<div class="container-fluid">
				...
				<div class="col-12 col-sm-12 tabs">
					<ul>
						<li><a href="<?php echo base_url() ?>Admins">Admins (<?php echo $ShowAdmin->num_rows()?>)</a></li>
						<li class="tabs-active"><a href="<?php echo base_url() ?>AdminsArchived">Archived</a></li>
					</ul>
				</div>
				<div class="row rcontent">
					<div class="col-5 PrintPageName PrintOut">
						<i class="fas fa-info-circle"></i>
						<i>Found <?php echo $ShowAdmin->num_rows(); ?> admin<?php if($ShowAdmin->num_rows() != 1): echo 's'; endif;?> archived in the database.
						</i>
					</div>
							...
							<table id="ListAdmins" class="table PrintOut" style="width: 100%;">
								<thead>
									<tr class="text-center">
										...
										<th style="width: 325px;"> Archived Since </th>
										<th class="d-none"> Archived Since </th>
								...
								<tbody>
									<?php foreach ($ShowAdmin->result_array() as $row):
										...
										$isArchivedValid = false;
										if ($row['DateRemoved']) {
											$archivedDate = new DateTime($row['DateRemoved']);
											$archivedDay = $archivedDate->format('Y-m-d');
											$archivedDay = DateTime::createFromFormat('Y-m-d', $archivedDay)->format('F d, Y');
											$archivedHours = $archivedDate->format('h:i:s A');
											$archivedElapsed = Carbon::parse($archivedDate);
											$isArchivedValid = true;
										}
										...
										<tr class="table-row-hover">
											...
											<?php endif; ?>
											<?php if ($isArchivedValid): ?>
											<td class="text-center align-middle" data-toggle="tooltip" data-placement="top" data-html="true" title="<?php echo $archivedElapsed->diffForHumans(); ?>">
												<div class="d-none">
													<?php echo $row['DateRemoved']; ?>
												</div>
												<?php
													echo $archivedDay . '<br>' . $archivedHours;
												?>
											</td>
											<td class="text-center align-middle d-none">
												<?php echo $row['DateRemoved']; ?>
											...
											<td class="text-center align-middle PrintExclude">
												...
												<button type="button" class="btn btn-primary btn-sm w-100 admin-moreinfo-btn mb-1" data-toggle="modal" data-target="#AdminMoreInfoModal" data-adminid="<?php echo $row['AdminID']; ?>" data-adminimage="<?php echo $row['Image']; ?>" data-adminposition="<?php echo $row['Position']; ?>" data-adminpermissions="<?php echo $row['Permissions']; ?>" data-adminlastname="<?php echo $row['LastName']; ?>" data-adminfirstname="<?php echo $row['FirstName']; ?>" data-adminmiddlename="<?php echo $row['MiddleName']; ?>" data-adminnotes="<?php echo $row['Notes']; ?>" data-activityelapsed=""><i class="fas fa-address-card"></i> More Info</button>
												<?php if($this->Model_Security->CheckPermissions('AdminsEditing')): ?>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$(".nav-item a[href*='Admins']").addClass("nactive");
		// =====================
		var permissionsCart = [];
